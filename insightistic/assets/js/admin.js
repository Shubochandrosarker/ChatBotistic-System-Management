...
( function ( $ ) {
	...
	var dataLoaded     = { overview: false, gsc: false, psi: false, woo: false };
	...
	/* ================================================================== */
	/* COMMERCE (WooCommerce Intelligence Pro)                             */
	/* ================================================================== */
	 ...
	 */
	function addonErrorNotice( $card, msg ) {
		$card.find( '.isp-addon-error' ).remove();
		var $n = $( '<div class="isp-notice isp-notice-error isp-addon-error"></div>' ).text( msg );
			...
			$.ajax( {
				...
				success: function ( res ) {
					if ( res.success ) {
						...
						$notice.html( '<div class="isp-notice isp-notice-error">' + escHtml( res.data || insightisticPro.i18n.error ) + '</div>' );
					}
				},
			...
			$.ajax( {
				...
				success: function ( res ) {
					if ( res.success && res.data && res.data.html ) {
						...
						$target.html( '<div class="isp-notice isp-notice-error">' + escHtml( res.data || insightisticPro.i18n.error ) + '</div>' );
					}
				},
				error: function () {
