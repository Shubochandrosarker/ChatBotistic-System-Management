(function () {
	...
	function setCycle(root, cycle) {
		...
		prices.forEach(function (price) {
			if (price.getAttribute('data-cycle-mode') === 'lifetime') {
				price.textContent = '***';
				return;
			}
			if (price.getAttribute('data-price-mask') === '1') {
				price.textContent = '***';
				return;
			}
		...
		labels.forEach(function (label) {
			var price = label.parentNode ? label.parentNode.querySelector('.memberistic-price-value') : null;
			if (price && price.getAttribute('data-cycle-mode') === 'lifetime') {
