	...
	<?php else : ?>
		<div class="memberistic-plan-grid">
			<?php foreach ( $plans as $plan ) : ?>
				...
				$is_lifetime     = 'lifetime' === $slug || 'lifetime' === ( $settings['billing_cycle'] ?? '' );
				// Only contact-only plans (e.g. Lifetime) mask the price. A genuine
				// $0 plan like Free must show "$0", never "***".
				$is_contact_only = ! empty( $settings['contact_only'] );
				$is_masked_price = $is_contact_only;
				$is_free         = ! $is_contact_only && 0.0 === $monthly && 0.0 === $annual;
				// Contact-only plans route to the LTD inquiry, not free checkout.
				if ( $is_contact_only ) {
					$url = add_query_arg( array( 'plan' => $slug ), home_url( '/contact/' ) );
				} else {
					$url = $checkout_url ? add_query_arg( array( 'memberistic_plan' => $slug ), $checkout_url ) : '#';
				}
				...
				<article class="memberistic-plan-card<?php echo ! empty( $plan['is_featured'] ) ? ' is-featured' : ''; ?>">
					...
					<div class="memberistic-plan-price">
						<span class="memberistic-price-value<?php echo $is_contact_only ? ' memberistic-price-value--lifetime' : ''; ?>" data-monthly="<?php echo esc_attr( number_format( $monthly, 2, '.', '' ) ); ?>" data-annual="<?php echo esc_attr( number_format( $annual, 2, '.', '' ) ); ?>" data-price-mask="<?php echo $is_masked_price ? '1' : '0'; ?>" data-cycle-mode="<?php echo $is_contact_only ? 'lifetime' : 'recurring'; ?>">
							<?php echo $is_masked_price ? esc_html( (string) ( $settings['price_display'] ?? '***' ) ) : '$' . esc_html( number_format_i18n( $monthly, 2 ) ); ?>
						</span>
						<small class="memberistic-price-cycle">
							<?php
							if ( $is_contact_only ) {
								esc_html_e( '/ lifetime', 'memberistic' );
							} elseif ( $is_free ) {
								esc_html_e( '/ forever', 'memberistic' );
							} else {
								esc_html_e( '/ month', 'memberistic' );
							}
							?>
						</small>
					</div>
					<?php if ( ! $is_contact_only && ! $is_free && $annual_savings > 0 ) : ?>
					...
					<a class="memberistic-plan-button" href="<?php echo esc_url( $url ); ?>">
						<?php
						if ( $is_contact_only ) {
							esc_html_e( 'Contact for LTD Pricing', 'memberistic' );
						} elseif ( $is_free ) {
							esc_html_e( 'Get started free', 'memberistic' );
						} else {
							echo esc_html( sprintf( __( 'Choose %s', 'memberistic' ), $plan['name'] ) );
						}
						?>
					</a>
				</article>
